  ...
  useEffect(() => {
    if (selectedConversation?._id) {
      const checkBlockStatus = async () => {
        try {
          const response = await axios.get('/api/user/checkBlockStatus', {
            params: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if(error.status=403){
        ...
        // alert("You cannot send a message. The user has blocked you.");
  ...
  const handleBlockUnblock = async () => {
    try {
      if (isBlocked) {
        ...
        setIsBlocked(false); // Update state to unblock
        ...
        setIsBlocked(true); // Update state to block
      ...
      alert('An error occurred during block/unblock action.');
    }
  };
                    ...
                    <div className='w-full rounded-full flex items-center bg-white'>
                        ...
                        <button type='submit'>
                            {sending ? <div className='loading loading-spinner'></div> :
                                <IoSend size={25} className='text-emerald-700 cursor-pointer rounded-full bg-gray-800 w-10 h-auto p-1' />
                            }
                        </button>
                    </div>
                </form>
